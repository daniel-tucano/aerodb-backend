...
describe("projectController tests", () => {
  beforeAll(async () => {
    ...
    await request(app.express)
      .post("/airfoils")
      .auth(JWTMocks.user_1, { type: "bearer" })
      .send(airfoilMocks.authorizedAirfoil_2);
    await request(app.express)
      .post("/runs")
      .auth(JWTMocks.user_1, { type: "bearer" })
    ...
    await request(app.express)
      .post("/runs")
      .auth(JWTMocks.user_1, { type: "bearer" })
      .send(runMocks.authorizedRun_2);
    await request(app.express)
      .post("/runs")
      .auth(JWTMocks.user_1, { type: "bearer" })
      .send({ ...runMocks.authorizedRun, airfoilID: 2 });
    await request(app.express)
      .post("/runs")
      .auth(JWTMocks.user_1, { type: "bearer" })
      .send({ ...runMocks.authorizedRun_2, airfoilID: 2 });
    await request(app.express)
      .post("/runs")
  ...
  it("Should authorize to get an project", async () => {
    const insertRes = await request(app.express)
      .post("/projects")
      .auth(JWTMocks.user_1, { type: "bearer" })
      .send(projectMocks.authorizedProject);
    const res = await request(app.express)
      .get(`/projects/${insertRes.body._id}`)
      .auth(JWTMocks.user_1, { type: "bearer" });
  ...
  it("Should unauthorize to insert the project document with uid diferent then authenticated user uid", async () => {
    const res = await request(app.express)
      .post("/projects")
      .send(projectMocks.unauthorizeduidProject)
      .auth(JWTMocks.user_1, { type: "bearer" });
  ...
  it(`Should invalidate insert project request with invalid airfoil data`, async () => {
    const res = await request(app.express)
      .post("/projects")
      .auth(JWTMocks.user_1, { type: "bearer" })
      .send(projectMocks.invalidAirfoilProject);
  ...
  it(`Should invalidate insert project request with invalid run data`, async () => {
    const res = await request(app.express)
      .post("/projects")
      .auth(JWTMocks.user_1, { type: "bearer" })
      .send(projectMocks.invalidRunProject);

    expect(res.status).toBe(400);
  });

  it("Should delete project document", async () => {
    const insertedProjectRes = await request(app.express)
      .post("/projects")
      .auth(JWTMocks.user_1, { type: "bearer" })
      .send(projectMocks.authorizedProject);
    const res = await request(app.express)
      .delete(`/projects/${insertedProjectRes.body._id}`)
      .auth(JWTMocks.user_1, { type: "bearer" });
  ...
  it("Should not find project document to delete", async () => {
    const res = await request(app.express)
      .delete(`/projects/5f973761a0eac4c121b3975b`)
      .auth(JWTMocks.user_1, { type: "bearer" });
  ...
  it("Should update project document", async () => {
    const insertedProjectRes = await request(app.express)
      ...
      .send(projectMocks.authorizedProject);
    const res = await request(app.express)
      .put(`/projects/${insertedProjectRes.body._id}`)
      .auth(JWTMocks.user_1, { type: "bearer" })
      .send(projectMocks.updatedProject);
  ...
  it("Should not find project document to update", async () => {
    const res = await request(app.express)
      .put(`/projects/5f973761a0eac4c121b3975b`)
      .auth(JWTMocks.user_1, { type: "bearer" })
      .send(projectMocks.updatedProject);
  ...
  it("Should unauthorize to update project creator field", async () => {
    ...
    const res = await request(app.express)
      .put(`/projects/${insertedProjectRes.body._id}`)
      .auth(JWTMocks.user_1, { type: "bearer" })
      .send(projectMocks.unauthorizedCreatorProject);
  ...
  it("Should add project to user projects when insert project document", async () => {
    const insertedProjectRes = await request(app.express)
      ...
      .auth(JWTMocks.user_1, { type: "bearer" })
      .send(projectMocks.authorizedProject);
    const userRes = await request(app.express)
      .get(`/users/${insertedProjectRes.body.creator.uid}?uid=true`)
      .auth(JWTMocks.user_1, { type: "bearer" });

    expect(
      _.some(userRes.body.projects, {
        name: insertedProjectRes.body.name,
        projectID: insertedProjectRes.body._id,
      })
    ).toBe(true);
  });
  ...
  it("Should return a pagination with 5 project documents", async () => {
    ...
    const projectPaginationRes = await request(app.express)
