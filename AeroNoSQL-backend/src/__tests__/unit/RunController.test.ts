...
describe('runController tests', () => {
    ...
    it('Should authorize to insert the run document', async () => {
        const res = await request(app.express).post('/runs').auth(JWTMocks.user_1, { type: 'bearer' }).send(runMocks.authorizedRun)

        expect(res.status).toBe(200)
    })

    it('Should unauthorize to insert the run document', async () => {
        const res = await request(app.express).post('/runs').send(runMocks.unauthorizedRun).auth(JWTMocks.user_1, { type: "bearer" })

        expect(res.status).toBe(401)
    })

    it('Should delete run document', async () => {
        const insertedRunRes = await request(app.express).post('/runs').auth(JWTMocks.user_1, { type: 'bearer' }).send(runMocks.authorizedRun)
        const res = await request(app.express).delete(`/runs/${insertedRunRes.body.runID}`).auth(JWTMocks.user_1, { type: 'bearer' })
        
        expect(res.status).toBe(200)
    })
    
    it('Should not find run document to delete', async () => {
        const res = await request(app.express).delete(`/runs/1`).auth(JWTMocks.user_1, { type: 'bearer' })
    ...
    it('Should unauthorize to delete run document', async () => {
        const insertedRunRes = await request(app.express).post('/runs').auth(JWTMocks.user_1, { type: 'bearer' }).send(runMocks.authorizedRun)
        const res = await request(app.express).delete(`/runs/${insertedRunRes.body.runID}`).auth(JWTMocks.user_2, { type: 'bearer' })
    ...
    it('Should update run document', async () => {
        const insertedRunRes = await request(app.express).post('/runs').auth(JWTMocks.user_1, { type: 'bearer' }).send(runMocks.authorizedRun)
        const res = await request(app.express).put(`/runs/${insertedRunRes.body.runID}`).auth(JWTMocks.user_1, { type: 'bearer' }).send(runMocks.updatedRun)
        
        expect(res.body).toMatchObject(runMocks.updatedRun)
    })
    
    it('Should not find run document to update', async () => {
        const res = await request(app.express).put(`/runs/1`).auth(JWTMocks.user_1, { type: 'bearer' }).send(runMocks.updatedRun)
    ...
    it("Should unauthorize to update run 'creator' field", async () => {
        const insertedRunRes = await request(app.express).post('/runs').auth(JWTMocks.user_1, { type: 'bearer' }).send(runMocks.authorizedRun)
        const res = await request(app.express).put(`/runs/${insertedRunRes.body.runID}`).auth(JWTMocks.user_1, { type: 'bearer' }).send(runMocks.unauthorizedUpdatedRun)
    ...
    it('Should add run to airfoil runs when insert run document', async () => {
        const insertedRunRes = await request(app.express).post('/runs').auth(JWTMocks.user_1, { type: 'bearer' }).send(runMocks.authorizedRun)
        const airfoilRes = await request(app.express).get(`/airfoils/${insertedRunRes.body.airfoilID}`).auth(JWTMocks.user_1, { type: 'bearer' })
    ...
    it('Should remove run from airfoil runs when delete run document', async () => {
        const insertedRunRes = await request(app.express).post('/runs').auth(JWTMocks.user_1, { type: 'bearer' }).send(runMocks.authorizedRun)
        await request(app.express).delete(`/runs/${insertedRunRes.body.runID}`).auth(JWTMocks.user_1, { type: 'bearer' })
        const airfoilRes = await request(app.express).get(`/airfoils/${insertedRunRes.body.airfoilID}`).auth(JWTMocks.user_1, { type: 'bearer' })

        expect(airfoilRes.body.runs.runIDs).not.toContain(insertedRunRes.body.runID)
    })
