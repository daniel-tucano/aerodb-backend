...
describe('UserController tests', () => {
    ...
    it('Should insert user document', async () => {
        const res = await request(app.express).post('/users').send(userMocks.user).auth(JWTMocks.user_1, { type: 'bearer' })
    ...
    it('Should not find user document to delete', async () => {
        const res = await request(app.express).delete(`/users/${userMocks.uid}`).auth(JWTMocks.user_1, { type: 'bearer' })
    ...
    it('Should not find user document to update', async () => {
        const res = await request(app.express).put(`/users/${userMocks.uid}`).auth(JWTMocks.user_1, { type: 'bearer' }).send(userMocks.updatedUser)
    ...
    it('Should return a pagination with 5 user documents', async () => {
        for (let i = 0; i < 10; i++) {
            await request(app.express).post('/users').auth(JWTMocks.user_1, { type: 'bearer' }).send(userMocks.user)
        }
        const userPaginationRes = await request(app.express).get('/users?limit=5').auth(JWTMocks.user_1, { type: 'bearer' })
    ...
    it('Should return a pagination with second page', async () => {
        ...
        await request(app.express).post('/users').auth(JWTMocks.user_1, { type: 'bearer' }).send({...userMocks.user, name: "user 11"})
        const userPaginationRes = await request(app.express).get('/users?page=2').auth(JWTMocks.user_1, { type: 'bearer' })
