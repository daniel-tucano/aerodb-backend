...
module.exports = {
    ...
    async store(req: Request<any, ProjectDataType, ProjectDataType>, res: Response) {
        ...
        for (const projectAirfoil of req.body.airfoils) {
            ...
            const airfoil = await Airfoils.findOne({ airfoilID: projectAirfoil.airfoilID }, {}, { lean: true })
            // If no airfoil is returned from database then sends an error
            if (!airfoil) return res.status(400).send(`AIRFOIL ${projectAirfoil.airfoilID} NOT FOUND!`)
            ...
            if (!lodash.some([airfoil], projectAirfoilCommonValues)) return res.status(400).send(`PROJECT PROVIDED AIRFOIL ${airfoil.airfoilID} IS INCONSISTENT WITH DATABASE VALUES`)
            ...
            for (const projectRun of projectAirfoil.runsData) {
                ...
                const run = await Runs.findOne({ runID: projectRun.runID }, {}, { lean: true })
                // If no run is returned from database then sends an error
                if (!run) return res.status(400).send(`RUN ${projectRun.runID} NOT FOUND!`)
                // If the common values from the project run differ from the database run values then sends an error
                if (!lodash.some([run], projectRun)) return res.status(400).send(`PROJECT PROVIDED RUN ${run.runID} IS INCONSISTENT WITH DATABASE VALUES`)
            }
        }
    ...
    async update(req: Request<any, ProjectDataType, ProjectDataType>, res: Response) {
        const project = await Project.findById(req.params.id, {}, { lean: true })
        ...
        // If it is authorized, perform the operation and return its result
        const project_new = await Project.findByIdAndUpdate(req.params.id, req.body, { new: true, useFindAndModify: false, lean: true })
