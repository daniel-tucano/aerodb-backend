...
module.exports = {
  async index(req: Request, res: Response) {
    ...
    const runs = await paginate(Run, req.ODataFilter, req.ODataSort, {
      page,
      limit,
  ...
  async store(req: Request<any, RunDataType, RunDataType>, res: Response) {
    ...
    const airfoil = await Airfoil.findOne(
      { airfoilID: req.body.airfoilID },
      {},
      { lean: true }
    );
    ...
    const runCounter = await Counter.findOneAndUpdate(
      { refCollection: "Runs" },
      { $inc: { counter: 1 } },
    ...
    if (run) {
      await Airfoil.findOneAndUpdate(
        { airfoilID: run.airfoilID },
        { $addToSet: { "runs.runIDs": run.runID, "runs.runObjIDs": run._id } },
  ...
  async update(req: Request<any, RunDataType, RunDataType>, res: Response) {
    ...
    // Checks if the operation is authorized
    if (req.decodedIdToken?.uid !== run.creator.uid)
      return res.status(401).send("CLIENT NOT AUTHORIZED TO PERFORM OPERATION");

    // Checks if it's trying to change creator field
    if (!lodash.isEqual(req.body.creator, run.creator))
      return res
        .status(401)
        .send(
  ...
  async destroy(req: Request, res: Response) {
    ...
    // If it is authorized, perform the operation and return its result
    const run_deleted = await Run.findOneAndDelete(
      { runID: Number(req.params.id) },
      { useFindAndModify: false } as QueryFindOneAndRemoveOptions
    );

    // If successfully deleted the run, then update the airfoil to reflect the deletion
    if (run_deleted) {
      await Airfoil.findOneAndUpdate(
        { airfoilID: run_deleted.airfoilID },
        {
          $pull: {
            "runs.runIDs": run_deleted.runID,
            "runs.runObjIDs": run_deleted._id,
          },
        },
        { useFindAndModify: false }
      );
